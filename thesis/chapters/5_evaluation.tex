...
\todo{übersicht}
You need to describe the datasets somewhere, including a) how they are generated or where do they come from (there are papers for the first three datasets) and b) what are their features (e.g., \# triples, \# resources, etc.)
Especially the ELR and SPS values should be listed as well.
...
The previous chapter used synthetic data to detect the behavior of the compressors to certain changes in the structure of the input data. First, real data will be used to compare the compression ratios of \GHDT{} and \GGRP{}. Thus, several datasets will be used (DBPedia, Semantic Web Dog Food, WikiData and Open Government Data).
...
Fig.~\ref{fig:compare1} and~\ref{fig:compare2} show the compression ratios for the datasets, respectively. It has to be noticed that values of $CR_{Graph_C}$ are not comparable between different RDF files. This is due to the fact that $CR_{Graph_C}$ does not take the dictionary into account. Hence, if two files $f_1,f_2$ have the same graph structure, but $f_1$  is using longer URIs than $f2$, then $CR_{Graph_C}$ will be smaller for $f_1$. This is because $|f_1|>|f_2|$ is true and $CR_{Graph_C}=\frac{in}{out_{graph}}$ (with $in\in \{f_1,f_2\}$ in this case). Therefore, the compression ratios of the two approaches can only be compared if they were achieved on the same input file.
...
\begin{center}
	\begin{tabular}{|c|c|c|c|c|c|}
		...
		\hline
		DF0 & dc-2010-complete-alignments & 5919 & 821 & 0.008 & 0.0 \\
		\hline
		...
		\hline 
	\end{tabular} 
	\captionof{table}{Datasets for the comparison of GRP and HDT.}	
...
The opposing approach (removing triples with inverse properties) is not explicitly evaluated here, because in Fig.~\ref{fig:inverseAddResults} and~\ref{fig:inverseAddResultsBigger} it is already recognizable that a graph with less inverse pairs of triples produces a worse compression ratio.\todo{inverse entfernen machen}
...
\begin{figure}[h]
	...
	\caption{Compression ratios~(\ref{fig:dogfoodcomprratiosSub}) and features of the same files~(\ref{fig:dogfoodliteralanalysis}).}
	\label{fig:dogfood}
\end{figure}
\todo{std deviations}
