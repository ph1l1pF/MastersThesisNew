...
Generally, it can be said that GRP can compress a graph better if $elr$ is lower, because then it is more likely that there is much redundancy in the graph. However, if the graph structure becomes unfavorable for GRP, the compression ratio may still be worse at a lower value for $elr$.
