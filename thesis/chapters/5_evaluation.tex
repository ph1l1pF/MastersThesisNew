...
The just described effect also sophisticates the Spearman correlation values to some extent. However, due to the fact that the \CRGraph{C}-values do not fluctuate so much, the correlation values still provide a valid indication. Moreover, it is reasonable to compare the different correlation values with each other. For instance, it becomes clear that \GGRP{} has a positive correlation with $ELR$ and a negative correlation with $SPS$ and the correlation with $ELR$ is much higher in terms of their absolute values. 

$CR_{Graph_{HDT}}$ has a positive correlation with $ELR$, but that correlation is quite small. The correlation between $CR_{Graph_{HDT}}$ and $SPS$ is positive (but very close  to 0) which is reasonable, since $Graph_{HDT}$ only benefits from the hub pattern, but $SPS$ is defined for the general star pattern.
...
\begin{figure}[h]
	\centering
	\subfloat[Results for adding symmetric properties.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratiosSymmetricsAdd}\label{fig:symmetricAddResults}}
	\hfill
	\subfloat[Results for deleting symmetric properties.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratiosSymmetricsDelete}\label{fig:symmetricDeleteResults}}
	\caption{Results for adding or deleting symmetric properties, on the grammar level ($IER$, $OER$) and on the file size level ($SR$).}
	\label{}
\end{figure}
...
\begin{figure}[h]
	\centering
	\subfloat[Results for adding inverse properties.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratiosInverseAdd}\label{fig:inverseAddResults}}
	\hfill
	\subfloat[Results for adding inverse properties in DBPedia (bigger graph than in Fig.~\ref{fig:inverseAddResults}).]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratiosInverseAddBigger}\label{fig:inverseAddResultsBigger}}
...
\begin{figure}[h]
	\centering
	\subfloat[Results for removing transitive properties.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratioTransitivesDelete}\label{fig:ratiotransitivesDelete}}
	\hfill
	\subfloat[Results for adding transitive properties.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/ontology/ratioTransitivesAdd}\label{fig:ratiotransitivesAdd}}
...
Thus, the results of the evaluation are presented. First, the Semantic Web Dog Food data set is used. This is well suited for an evaluation, since both literals and URIS are included here. Fig.~\ref{fig:dogfoodcomprratiosSub} shows \CRDict{HDT} for a subset of the data (the same files as in Fig.~\ref{fig:figcompare1b}).
...
\begin{figure}[h]
	\centering
	\subfloat[\CRDict{HDT} for Normal HDT and HDT + Huffman]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/dogFoodComprRatios}\label{fig:dogfoodcomprratiosSub}}
	\hfill
...
\begin{figure}[h]
	\centering
	\subfloat[\CRDict{HDT} for Normal HDT and HDT + Huffman.]{\includegraphics[width=\subfigWidth\textwidth]{figures/4_evaluation/dbAbstractsComprRatios}\label{fig:dbAbstractscomprratiosSub}}
	\hfill
