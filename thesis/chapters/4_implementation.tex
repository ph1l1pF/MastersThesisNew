...
This chapter is about how to manipulate the RDF graphs to match the properties of Ch.~\ref{sec:approachOntKnowledge}. For this the query language SPARQL~\cite{sparql} was used.

The implementation works in such a way that the individual features are first executed on separate graphs. So there is a graph where all symmetric edges are added or removed, and analogously for the other manipulations. Finally, there is also a graph on which all manipulations are performed. In this way one sees first, what the manipulations bring individually and in the end also, what they all achieve in combination.
...
\begin{lstlisting}[captionpos=b, caption=SPARQL query for removing triples with the transitive property p, label=lst:transitiveRemove,
basicstyle=\ttfamily,frame=single]
DELETE { ?s ?p ?o }
WHERE { 
	?s ?p/?p+ ?o. 
